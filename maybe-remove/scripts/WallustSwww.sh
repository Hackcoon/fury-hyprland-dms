...
read_cached_wallpaper() {
  ...
  if [[ -f "$cache_file" ]]; then
    # awww/swww cache format: "crop:...Lanczos3<path>" (one null-separated line)
    # strip everything before the last path start — robust in both formats
    tr -d '\0' < "$cache_file" | awk 'NF {print; exit}' | grep -oE '/[^ ]+\.(jpg|jpeg|png|webp|gif|bmp)$'
  fi
}
